...
class Controller:
    def __init__(self, name):
        ...
        rospy.Subscriber('/yolo_result', Point, self.transform, queue_size=1)
    ...
    def control(self, place):
        ...
        print('I will go near the ' + self.result)
        self.soundplayer.say('I will go near the ' + self.result, 3)

        self.navigator.go_near(self.result, self.transformed_position, self.base.quaternion)
        print('Task completed!')
        self.soundplayer.say('Task completed.')

    def transform(self, point):
        """yolo_result的回调函数,实时更新检测到的物品名"""
        self.result = point.result
        self.transformed_position = self.transformer.transformed('kinect', 'map', point)
        rospy.loginfo(self.transformed_position)
