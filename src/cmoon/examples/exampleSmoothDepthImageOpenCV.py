...
if __name__ == "__main__":
    ...
    while True:
        ...
        if depth_image_handle:
            ...
            smooth_depth_color_image = cv2.applyColorMap(np.round(smoothed_depth_image / 30).astype(np.uint8),
                                                         cv2.COLORMAP_JET)

            # Concatenate images for comparison
            comparison_image = np.concatenate((depth_color_image, smooth_depth_color_image), axis=1)
            comparison_image = cv2.putText(comparison_image, 'Original', (180, 50), cv2.FONT_HERSHEY_SIMPLEX, 1.5,
                                           (255, 255, 255), 3, cv2.LINE_AA)
            comparison_image = cv2.putText(comparison_image, 'Smoothed', (670, 50), cv2.FONT_HERSHEY_SIMPLEX, 1.5,
                                           (255, 255, 255), 3, cv2.LINE_AA)
