...
class Stm32:
    ...
    def connect(self):
        try:
            print
            "Connecting to Stm32 on port", self.port, "..."
        ...
        try:
            ...
            while attempts < ntries and res != 1:
                try:
                    ...
                    print
                    "Exception executing command: " + str(binascii.b2a_hex(cmd))
                attempts += 1
        except:
            self.mutex.release()
            print
    ...
    def drive(self, left, right):
        ...
        cmd_str = struct.pack("4B", self.HEADER0, self.HEADER1, 0x05, 0x04) + struct.pack("hh", left,
                                                                                          right) + struct.pack("B",
                                                                                                               self.check_num)
        if (self.execute(cmd_str)) == 1 and self.payload_ack == '\x00':
            return self.SUCCESS
    ...
    def set_pid(self, cmd, left, right):
        ...
        cmd_str = struct.pack("8B", self.HEADER0, self.HEADER1, 0x05, cmd, lpid_h, lpid_l, rpid_h,
                              rpid_l) + struct.pack("B", checknum)
        if (self.execute(cmd_str)) == 1 and self.payload_ack == '\x00':
            return self.SUCCESS
        else:
            return self.FAIL
    ...
    def get_infrared(self, ir_id):
        ...
        checknum = self.get_check_sum(check_number_list)
        cmd_str = struct.pack("6B", self.HEADER0, self.HEADER1, 0x03, 0x0E, ir_list[ir_id], 0x00) + struct.pack("B",
                                                                                                                checknum)
        if (self.execute(cmd_str)) == 1 and self.payload_ack == '\x00':
...
class BaseController:
    def __init__(self, Stm32, base_frame):
        ...
        self.safe_range_0 = rospy.get_param('sonar_safe_distance', 10.0)
        self.safe_range_1 = rospy.get_param('sonar_dec_distance', 40.0)
        ...
        if now > self.t_next:
            ...
            if (self.useSonar == True):
                ...
                try:
                    ...
                    if sonar0_range.range >= 0.5 or sonar0_range.range == 0.0:
                        self.sonar_cloud[0][0] = self.sonar0_offset_x + self.sonar_maxval * math.cos(
                            self.sonar0_offset_yaw)
                        self.sonar_cloud[0][1] = self.sonar0_offset_y + self.sonar_maxval * math.sin(
                            self.sonar0_offset_yaw)
                    else:
                        self.sonar_cloud[0][0] = self.sonar0_offset_x + sonar0_range.range * math.cos(
                            self.sonar0_offset_yaw)
                        self.sonar_cloud[0][1] = self.sonar0_offset_y + sonar0_range.range * math.sin(
                            self.sonar0_offset_yaw)
                    ...
                    if sonar1_range.range >= 0.5 or sonar1_range.range == 0.0:
                        self.sonar_cloud[1][0] = self.sonar1_offset_x + self.sonar_maxval * math.cos(
                            self.sonar1_offset_yaw)
                        self.sonar_cloud[1][1] = self.sonar1_offset_y + self.sonar_maxval * math.sin(
                            self.sonar1_offset_yaw)
                    else:
                        self.sonar_cloud[1][0] = self.sonar1_offset_x + sonar1_range.range * math.cos(
                            self.sonar1_offset_yaw)
                        self.sonar_cloud[1][1] = self.sonar1_offset_y + sonar1_range.range * math.sin(
                            self.sonar1_offset_yaw)
                    ...
                    if sonar2_range.range >= 0.5 or sonar2_range.range == 0.0:
                        self.sonar_cloud[2][0] = self.sonar2_offset_x + self.sonar_maxval * math.cos(
                            self.sonar2_offset_yaw)
                        self.sonar_cloud[2][1] = self.sonar2_offset_y + self.sonar_maxval * math.sin(
                            self.sonar2_offset_yaw)
                    else:
                        self.sonar_cloud[2][0] = self.sonar2_offset_x + sonar2_range.range * math.cos(
                            self.sonar2_offset_yaw)
                        self.sonar_cloud[2][1] = self.sonar2_offset_y + sonar2_range.range * math.sin(
                            self.sonar2_offset_yaw)
                    ...
                    if sonar3_range.range >= 0.5 or sonar3_range.range == 0.0:
                        self.sonar_cloud[3][0] = self.sonar3_offset_x + self.sonar_maxval * math.cos(
                            self.sonar3_offset_yaw)
                        self.sonar_cloud[3][1] = self.sonar3_offset_y + self.sonar_maxval * math.sin(
                            self.sonar3_offset_yaw)
                    else:
                        self.sonar_cloud[3][0] = self.sonar3_offset_x + sonar3_range.range * math.cos(
                            self.sonar3_offset_yaw)
                        self.sonar_cloud[3][1] = self.sonar3_offset_y + sonar3_range.range * math.sin(
                            self.sonar3_offset_yaw)
                    ...
                    if sonar4_range.range >= 0.5 or sonar4_range.range == 0.0:
                        self.sonar_cloud[4][0] = self.sonar4_offset_x + self.sonar_maxval * math.cos(
                            self.sonar4_offset_yaw)
                        self.sonar_cloud[4][1] = self.sonar4_offset_y + self.sonar_maxval * math.sin(
                            self.sonar4_offset_yaw)
                    else:
                        self.sonar_cloud[4][0] = self.sonar4_offset_x + sonar4_range.range * math.cos(
                            self.sonar4_offset_yaw)
                        self.sonar_cloud[4][1] = self.sonar4_offset_y + sonar4_range.range * math.sin(
                            self.sonar4_offset_yaw)
            ...
            if (self.useImu == True):
                try:
                    ...
                    # imu_quaternion.z = sin(self.imu_offset*yaw*3.1416/(180 *2.0))
                    # imu_quaternion.w = cos(self.imu_offset*yaw*3.1416/(180 *2.0))
                    ...
                    # self.imuAnglePub.publish(self.imu_offset*yaw*3.1416/(180 *2.0))
    ...
    def cmdVelCallback(self, req):
        ...
        if (self.useSonar == True):
            if (self.sonar_r1 > self.safe_range_0 and self.sonar_r1 <= self.safe_range_1 and th < 0):
                temp = self.sonar_r1 / (self.safe_range_1 * 1.5)
                if (th < -0.05):
                    th = temp * th
            if (self.sonar_r2 > self.safe_range_0 and self.sonar_r2 <= self.safe_range_1 and x > 0):
                temp = self.sonar_r2 / (self.safe_range_1 * 1.5)
                if (x > 0.1):
                    x = temp * x
                if (x < 0.05):
                    x = 0.05
        ...
        if x == 0:
            # Turn in place
            '''
            if th>0.0 and th<self.start_rotation_limit_w:
            ...
            if (th>0.0 and th<self.start_rotation_limit_w) and (x>-0.1 and x<0):
                th=self.start_rotation_limit_w
	            #if (th>-0.4 and th<0.0) and (x>-0.1 and x<0.1):
